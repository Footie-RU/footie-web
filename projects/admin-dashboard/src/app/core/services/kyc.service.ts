...
export class KycService {
  ...
  uploadDocument(
    ...
    return this.httpClient
      .post<RequestResponse>(
        ApiEndpoints.users.kyc.uploadDocument(userId, fileField),
        file
      )
      .pipe(
        switchMap((response) =>
  ...
  verifyKYCDocuments(userId: string): Observable<RequestResponse> {
    return this.httpClient
      .patch<RequestResponse>(
        ApiEndpoints.users.kyc.verifyKYCDocuments(userId),
        {}
      )
      .pipe(
        switchMap((response) =>
  ...
  submitKYC(userId: string, formData: any): Observable<RequestResponse> {
    ...
    if (
      !formData.internationalPassport ||
      !formData.schoolID ||
      !formData.selfie
    ) {
      return throwError({
    ...
    internationalPassportFormData.append(
      'file',
      formData.internationalPassport
    );
    ...
    return this.uploadDocument(
      ...
      concatMap((passportResponse) => {
        if (passportResponse.result !== 'success') {
          return throwError({
            result: 'error',
            message: 'International Passport upload failed',
          });
        }
      ...
      concatMap((schoolIDResponse) => {
        if (schoolIDResponse.result !== 'success') {
          return throwError({
            result: 'error',
            message: 'School ID upload failed',
          });
        }
   ...
   */
  updateKYCRecord(userId: string, payload: any): Observable<RequestResponse> {
    return this.httpClient
      .patch<RequestResponse>(ApiEndpoints.users.kyc.update(userId), {
        payload,
      })
      .pipe(
        switchMap((response) =>
